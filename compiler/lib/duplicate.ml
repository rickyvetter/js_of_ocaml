...
let last m s l =
  ...
  | Pushtrap (cont1, x, cont2, pcs) ->
      Pushtrap
        ( subst_cont m s cont1
        , s x
        , subst_cont m s cont2
        , Addr.Set.map (fun pc -> Addr.Map.find pc m) pcs )
  | Return x -> Return (s x)
  ...
  | Switch (x, a1, a2) ->
      Switch
        ( s x
        , Array.map a1 ~f:(fun cont -> subst_cont m s cont)
        , Array.map a2 ~f:(fun cont -> subst_cont m s cont) )
